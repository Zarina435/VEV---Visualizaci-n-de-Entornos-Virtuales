...
//Funciones igual que en perfragment.frag
...
vec3 reflejo (){
	...
	//Producto
	float aux= dot(f_normalw,I);
	
	//Aplicamos formula
	R = 2*aux*f_normalw - I;

	//Cambiamos la Z de signo
	 R = vec3(R.xy,-R.z);
...
void main() {
	...
	vec4 texturaEnv;
	ref= reflejo();
	texturaEnv= textureCube(envmap, ref);

	gl_FragColor=vColor*texColor*texturaEnv;
